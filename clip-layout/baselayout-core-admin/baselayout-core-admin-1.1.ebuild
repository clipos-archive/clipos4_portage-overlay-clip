...
DESCRIPTION="Baselayout for admin view in CLIP core"
...
RDEPEND="!clip-deps? ( >=clip-layout/baselayout-clip-1.7.3 )"
...
src_install() {
	...
	keepdir /proc /dev /tmp /sys
	fperms 700 /root


	keepdir /usr/bin /home
	if [[ "${ARCH}" =~ ^.*64$ ]] ; then
