...
CONFIG_PVR="3.14"
PATCH_PVR="3.14.79-r3"
CLSM_PVR="3.14.79-r2"
I915_PVR="3.14.52"
PF_RING_PVR="6.0.3"
PF_RING_PATCHES_PVR="6.0.3-p2"

MY_PN=${PN/-kernel/-patches}
CLIP_PATCH="${MY_PN}-${PATCH_PVR}.tar.bz2"
CLSM_SRC="clip-lsm-${CLSM_PVR}.tar.bz2"
I915_PATCH="linux-${I915_PVR}-i915-backport.tar.bz2"
PF_RING_SRC="PF_RING-${PF_RING_PVR}.tar.gz"
PF_RING_PATCHES_SRC="pfring-kernel-patches-${PF_RING_PATCHES_PVR}.tar.xz"
...
IUSE="clip-devel clip-gtw clip-modular clip-xen
clip-devstation clip-livecd debug clip-light arm pf_ring"
...
SRC_URI="${KERNEL_URI} mirror://clip/${CLIP_PATCH}
			...
			mirror://clip/${I915_PATCH}
			pf_ring? (
				mirror://clip/${PF_RING_SRC}
				mirror://clip/${PF_RING_PATCHES_SRC}
			)
		"
...
src_prepare() {
	...
	# PF_RING
	if use pf_ring; then
		cd "${WORKDIR}/PF_RING-${PF_RING_PVR}"
		EPATCH_SOURCE="${WORKDIR}/pfring-kernel-patches-${PF_RING_PATCHES_PVR}/pfring-patches" \
			EPATCH_SUFFIX=patch epatch
		cd "${S}"
		mkdir "net/pf_ring"
		cp "${WORKDIR}/PF_RING-${PF_RING_PVR}/kernel/linux/pf_ring.h" include/linux
		cp "${WORKDIR}/PF_RING-${PF_RING_PVR}/kernel/pf_ring.c" net/pf_ring
		EPATCH_SOURCE="${WORKDIR}/pfring-kernel-patches-${PF_RING_PATCHES_PVR}/linux-patches" \
			EPATCH_SUFFIX=patch epatch
	fi

	use clip-devstation && epatch "${FILESDIR}/linux-3.10.1-asm-bitsperlong.patch"

	# check proper config
	einfo "checking proper config"
	local my_conf=modular-smp

	use clip-xen    && my_conf=xen
	...
	if use pf_ring; then
		ewarn "Setting PF_RING option"
		set_opt "CONFIG_PF_RING_SUPPORT" "m"
	fi
...
src_compile() {
	...
	env -i make ARCH=${ARCH} ${opt_MAKE} silentoldconfig || die "Make oldconfig failed" 
	eend $?
...
src_install() {
	...
	newsbin "${FILESDIR}/mkinitrd.clip-3.14" "mkinitrd.clip"
