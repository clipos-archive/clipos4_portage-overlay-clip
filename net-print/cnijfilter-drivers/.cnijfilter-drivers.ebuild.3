...
DESCRIPTION_FR="Pilotes d'imprimantes Canon Inkjet (IP4700, MP250, MP270, MP490, MP550, MP560, MP640)"
...
IUSE="+servicetools clip"
for pruse in ${PRINTER_USE[@]}; do
	IUSE="${IUSE} +${pruse}"
done
...
_dir_build() {
	...
	for d in ${dirs}; do
		...
		if [[ ${command} == econf* ]]; then
			${command} \
				--enable-binpath="${EPREFIX}${CPREFIX:-/usr}/bin" \
				--enable-libpath="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/bjlib"
		else
			${command}
		fi
		popd > /dev/null
...
src_prepare() {
	...
	if use clip; then
		find "${S}" -name "Makefile.am" \
			-exec sed -i \
				-e "s:/usr/lib:${CPREFIX:-/usr}/$(get_libdir):g" \
				-e "s:_LDFLAGS = :_LDFLAGS = @LDFLAGS@ :" \
					'{}' \;
	fi
...
src_configure() {
	...
	append-cppflags "-D_IPP_PRIVATE_STRUCTURES"
...
src_install() {
	local _libdir="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)"
	#local _libdir_pkg=libs_bin$(use amd64 && echo 64 || echo 32)
	local _libdir_pkg=libs_bin
	local _ppddir="${EPREFIX}${CPREFIX:-/usr}/share/cups/model"
